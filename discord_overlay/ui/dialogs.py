...
class AccuracyReminder(ctk.CTkToplevel):
    """Shown on every launch: the full list of setup rules that decide parse accuracy."""
    ...
    def __init__(self, parent, tips) -> None:
        ...
        ctk.CTkLabel(self, text="OCR ACCURACY TIPS", text_color=theme.ACCENT, font=theme.font(24, bold=True)).grid(
            row=0, column=0, padx=28, pady=(24, 2), sticky="w")
        ctk.CTkLabel(self, text="The parser only sees pixels. These settings decide whether it reads every line "
                                "or misses damage. Check them before each session.",
                     text_color=theme.MUTED, font=theme.font(13), justify="left", anchor="w", wraplength=620).grid(
            row=1, column=0, padx=28, pady=(0, 14), sticky="w")
        body = ctk.CTkScrollableFrame(self, fg_color=theme.PANEL_2, corner_radius=10)
