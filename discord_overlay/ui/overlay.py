"""Always-on-top timer surface: arrangeable, then locked click-through."""
...
class TimerOverlay(tk.Toplevel):
    ...
    def __init__(self, parent, geometry: str, geometry_changed: Callable[[str], None],
        ...
        self.geometry(geometry or "420x320+40+80")
        self.minsize(160, 50)
        ...
        self.body = ctk.CTkScrollableFrame(self, fg_color=theme.BG_DEEP, corner_radius=0,
    ...
    def _finish_preview(self) -> None:
        ...
        if self.winfo_exists() and self.cards:
            self.lock()
    ...
    def _show_scrollbar(self, visible: bool) -> None:
        try:
            ...
            scrollbar.grid() if visible else scrollbar.grid_remove()
    ...
    def _resize_move(self, event) -> None:
        if self.edit_mode and self._resize_origin:
            ...
            self.geometry(f"{max(280, width + event.x_root - start_x)}x{max(120, height + event.y_root - start_y)}")
